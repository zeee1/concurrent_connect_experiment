...
def merge_Client_PLT(dirName):
	...
	subDir_list = glob("./client10/"+dirName+"*/")
...
def reassemble_data(_data):
	...
	for key in _data.keys():
		...
		if splitDf_h1["protocol"].count() != 0:
			result_dict["http1"].append(splitDf_h1['networkTime'].mean())
		if splitDf_h2["protocol"].count() != 0:
			result_dict["http2"].append(splitDf_h2['networkTime'].mean())
...
def convertToDF(dict_data, testName):
	data = pd.DataFrame()
	data['networkTime'] = dict_data["http1"]
	data['protocol'] = "http1"
	data = data.append(pd.DataFrame({"networkTime":dict_data["http2"], "protocol" : "http2"}))
	data["testName"] = testName
	return data

def mergeDF(src, dest):
	if src['networkTime'].count() == 0:
		return dest
	elif dest['networkTime'].count() == 0:
		return src
	else:
...
def visualize_and_analyze(h1_10_h2_0, h1_7_h2_3, h1_5_h2_5, h1_3_h2_7, h1_0_h2_10):
	total_result_set = mergeDF(convertToDF(h1_10_h2_0, "h1:10_h2:0"), convertToDF(h1_7_h2_3, "h1:7_h2:3"))
	...
	ax.set_title("PLT of http1/http2 when multi-user = 10 on PLR + 5%")
...
def main():
	test_env = ["default", "del_100", "plr_2", "multi_conn"]
	test_case = ["h1_10_h2_0", "h1_7_h2_3", "h1_5_h2_5", "h1_3_h2_7", "h1_0_h2_10"]
	#test_case = ["h1_7_h2_3", "h1_5_h2_5", "h1_3_h2_7"]

	test_list = []
	for i in test_case:
		# merge all client's log of PLT on same test_case
		merged_client_data = merge_Client_PLT("default/"+i+"/")
		# sort by timestamp
	...
	visualize_and_analyze(test_list[0], test_list[1], test_list[2], test_list[3], test_list[4])
	#visualize_and_analyze(test_list[0], test_list[1], test_list[2], test_list[3], test_list[4])
